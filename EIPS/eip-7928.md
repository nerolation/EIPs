...
State-accessing opcodes perform validation in two phases:

- **Pre-state validation**: Conditions determinable without target state access:
    - Gas costs (memory expansion, base opcode cost, `access_cost`, value transfer cost). For the CALL family (`CALL`, `CALLCODE`, `DELEGATECALL`, `STATICCALL`), only the warm-equivalent (`WARM_STORAGE_READ_COST`) of `access_cost` is pre-state; the cold-access surcharge is deferred to post-state.
    - For `CALL`, `CALLCODE`, `DELEGATECALL`, `STATICCALL`: call stack depth limit not exceeded
    - For `CALL`/`CALLCODE` with `value > 0`: sender balance sufficiency (`sender_balance >= value`)
- **Post-state validation**: Gas costs paid after pre-state passes (cold-access surcharge for the target, account creation cost (`GAS_NEW_ACCOUNT`), [EIP-7702](./eip-7702.md) delegated address `access_cost`). Warming and BAL insertion of an address occur paired with its access-cost charge.

Pre-state validation MUST pass before any state access occurs. If pre-state validation fails, the target resource (address or storage slot) is never accessed and MUST NOT be included in the BAL. On a precondition failure (depth or balance), only the pre-state gas is consumed.

Once pre-state validation passes, post-state charges proceed sequentially; each access-cost charge is paired with warming and BAL insertion of the corresponding address, and an OOG prevents that pairing.
...
| `CALL` | `WARM_STORAGE_READ_COST` + `memory_expansion` + `GAS_CALL_VALUE` (if value > 0) |
| `CALLCODE` | `WARM_STORAGE_READ_COST` + `memory_expansion` + `GAS_CALL_VALUE` (if value > 0) |
| `DELEGATECALL` | `WARM_STORAGE_READ_COST` + `memory_expansion` |
| `STATICCALL` | `WARM_STORAGE_READ_COST` + `memory_expansion` |
...
`GAS_NEW_ACCOUNT` and `GAS_SELF_DESTRUCT_NEW_ACCOUNT` are post-state costs charged after the relevant address has already been added to the BAL and therefore do not affect BAL inclusion.

#### EIP-7702 Delegation

When a call target has an [EIP-7702](./eip-7702.md) delegation, the delegated address's `access_cost` is the post-state charge (above) paired with its warming and BAL insertion. If pre-state validation fails for the call, neither the original call target nor the delegated address is accessed or included. If pre-state passes (so the target is loaded to resolve the delegation) but the delegated address's `access_cost` OOGs, the original call target is included but the delegated address MUST NOT appear in the BAL.
