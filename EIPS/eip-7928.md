...
- **Pre-state validation**: Conditions determinable without target state access:
    - Gas costs (memory expansion, base opcode cost, warm/cold access cost, value transfer cost)
...
When a call target has an [EIP-7702](./eip-7702.md) delegation, the delegated address is accessed only if pre-state validation passes for the call and sufficient gas remains for the delegated address's own `access_cost`. If this gas check fails, the delegated address MUST NOT appear in the BAL; the original call target is still included, having been accessed to resolve the delegation. If pre-state validation fails for the call, neither the original call target nor the delegated address is accessed or included.
