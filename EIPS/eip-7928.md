...
This EIP introduces Block-Level Access Lists (BALs) that record all accounts and storage locations accessed during block execution, along with their post-execution values and gas used per transaction. BALs enable parallel disk reads, parallel transaction validation, and executionless state updates.
...
We introduce a new field to the block header, `block_access_list_hash`, which contains the Keccak-256 hash of the RLP-encoded block access list. When no state changes are present (empty block), this field is the hash of `keccak256(rlp.encode([[], []]))`.
...
The block body includes a `BlockAccessList` containing all account accesses, state changes, and gas used per transaction. This field is RLP-encoded as `[AccountChanges[], gas_used[]]`. When no state changes are present, this field is `rlp.encode([[], []])`.
...
# BlockAccessList: [List[AccountChanges], List[gas_used]]
# Contains account changes and gas_used values for each transaction
BlockAccessList = [List[AccountChanges], List[uint256]]
```
...
The `gas_used` array contains the gas consumed by each transaction in block order. Each entry represents the final gas used after refunds for the corresponding transaction at that index.
...
def execute_and_collect_accesses(block):
    ...
    for i, tx in enumerate(block.transactions):
        tx_gas_used = execute_transaction(tx)
        gas_used.append(tx_gas_used)
    ...
    return build_bal(accesses, gas_used)
...
def build_bal(accesses, gas_used):
    ...
    return [account_changes, gas_used]
```
...
```python
[
  # First element: List of AccountChanges
  [
  ...
  # Second element: List of gas_used values (one per transaction)
  [
    21000,   # Transaction 1: Alice sends 1 ETH to Bob
    150000   # Transaction 2: Charlie calls factory to deploy contract
  ]
]
```
