...
GasUsed = uint64  # Gas consumed by a transaction
...
# BlockAccessList: Gas used per transaction followed by AccountChanges
# First element: list of gas_used values (one per transaction, indexed by block_access_index - 1)
# Second element: list of all account changes
BlockAccessList = [List[GasUsed], List[AccountChanges]]
```
...
```python
[
    # Gas used per transaction (indexed by block_access_index - 1)
    [21000, 150000],  # gas_used: tx 0 = 21000, tx 1 = 150000
    [
...
6. **Gas used per transaction**: Including gas_used values enables independent validation of each transaction and enforcement of the block gas limit without re-executing transactions. Validators can verify that the cumulative gas matches the header's gas_used field and that the block gas limit is not exceeded.
...
- Gas used per transaction: ~1.6 KiB (2.2%)
