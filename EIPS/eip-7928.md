...
- **Pre-state validation**: Conditions determinable without target state access:
    - Gas costs without cold-access surcharge (memory expansion, base opcode cost, warm-equivalent access cost, value transfer cost)
...
Pre-state validation MUST pass before any state access occurs. If pre-state validation fails, the target resource (address or storage slot) is never accessed and MUST NOT be included in the BAL; only pre-state gas is consumed.
