...
We introduce a new field to the EL block header, `block_access_list_hash`, which contains the Keccak-256 hash of the RLP-encoded block access list. When no state changes are present, this field is `keccak256(rlp.encode([]))`.
...
class BlockHeader:
    ...
    block_access_list_hash: Hash32  # Keccak-256 hash of RLP-encoded BAL
```

The `BlockAccessList` is not included in the block body. The EL stores BALs separately and transmits them via the `ExecutionPayload` in the engine API. The BAL is RLP-encoded as a list of `AccountChanges`. When no state changes are present, this field is the empty RLP list `0xc0`, i.e. `rlp.encode([])`.
...
<<<<<<< HEAD
The `ExecutionPayload` is extended with the `blockAccessList` field containing the RLP-encoded BAL.
...
The CL stores the BAL root using SSZ merkleization (`hash_tree_root(blockAccessList)`) in the ExecutionPayload.

The EL stores BALs separately from the block body and provides them via:
- The `blockAccessList` field in the ExecutionPayload during block production/validation
- Optional retrieval methods for historical BALs (similar to `engine_getPayloadBodiesByHashV1`):
...
The EL MUST retain BALs for at least the duration of until the last finalized checkpoint to support synchronization from the latest finalized checkpoint performing re-execution on the EL. 
=======
...
>>>>>>> f6c05d88 (Update EIP-7928: Engine api clarification)

### State Transition Function

The state transition function must validate the BAL received in the ExecutionPayload:
...
def validate_block(execution_payload, block_header):
    ...
    import rlp
    block_header.block_access_list_hash = keccak(rlp.encode(execution_payload.blockAccessList))
    ...
    # If this fails, the block is invalid (and the hash in the header would be wrong)
    assert rlp.encode(actual_bal) == rlp.encode(execution_payload.blockAccessList)
...
3. **Overhead analysis**: Historical data shows ~70 KiB average BAL size at a block gas limit of 60 million
...
Block size impact (historical analysis at 36 million gas limit):
