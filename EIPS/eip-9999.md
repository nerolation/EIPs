...
class ExecutionChunkHeader(Container):
    ...
    chunk_access_list_hash: Root    # Should we have this here?
    is_last: bool

class ExecutionHeaderMetadata(Container):
    parent_hash: Root
    number: uint64
    timestamp: uint64
    mix_hash: Bytes32
    base_fee_per_bas: Uint
    parent_beacon_block_root: Root

# Self-contained execution chunk
class ExecutionChunk(Container):
    chunk_header: ExecutionChunkHeader
    block_metadata: ExecutionHeaderMetadata # I believe
...
2. **Minimum Fill**: Non-terminal chunks MUST use at least `MIN_CHUNK_FILL_RATIO * CHUNK_GAS_LIMIT` gas (8,388,608 gas), except if the subsequent chunk exceeds the miniumum fill ratio again.
    - Alternatively, we could have a rule that is something like this: 
    
        Any particular chunk can have any amount of used gas, but the sum of every two consecutive chunks MUST exceed the `CHUNK_GAS_LIMIT` (effectively, it shouldn't be able to simply just merge two consecutive chunks without breaking rule 1.)
...
```python
# TODO: this is missing pre/post tx execution logic (only withdrawals logic is present, but there is more)
...
```python
# TODO: also here, CAL should include pre/post tx access/writes as well
...
**TODO:** We should define changes to engine_getPayload method as well, that allows for partial creation.
We might have to change return type and/or add parameters.

For example, we could:
- add following method `getChunkPayload(payload_id) -> ChunkPayload[]`
    - this would make EL commit to returned chunks. subsequent calls would assume that returned chunks are appended on the previously returned list
    - this method would never "finalized" the block (meaning, we will never include withdrawals and do post-tx execution)
- reuse `getPayload` which will "finalize" the block (create remaning chunks, and last chunk will do other post-tx execution and include withdrawals)
    - this method would return `Header` together with those remaining chunks
