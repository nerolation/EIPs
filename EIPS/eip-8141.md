...
category: Core
created: 2026-01-29
requires: 1559, 2718, 4844, 7976, 7997
---
...
```
tx_gas_limit = FRAME_TX_INTRINSIC_COST + len(tx.frames) * FRAME_TX_PER_FRAME_COST + max(
    calldata_cost(rlp(tx.frames)) + sum(frame.gas_limit for all frames),
    floor_calldata_cost(rlp(tx.frames))
)
```

Where `calldata_cost` is calculated per standard EVM rules (4 gas per zero byte, 16 gas per non-zero byte), and `floor_calldata_cost` is `TOTAL_COST_FLOOR_PER_TOKEN * floor_tokens_in_calldata` applied to the byte serialization of `rlp(tx.frames)`, as defined in [EIP-7976](./eip-7976.md). The `max` ensures frame transactions cannot be used to bypass the calldata floor cost applied to other transaction types.
...
Each frame has its own `gas_limit` allocation. Unused gas from a frame is **not** available to subsequent frames. After all frames execute, the transaction's effective gas used and the refund are calculated as:

```
tx_gas_used = FRAME_TX_INTRINSIC_COST + len(tx.frames) * FRAME_TX_PER_FRAME_COST + max(
    calldata_cost(rlp(tx.frames)) + total_gas_used,
    floor_calldata_cost(rlp(tx.frames))
)
refund = tx_gas_limit - tx_gas_used
```

When execution gas is high relative to calldata, the standard path (`calldata_cost + total_gas_used`) dominates the `max` and the floor is inert. When execution is low relative to calldata, the [EIP-7976](./eip-7976.md) floor sets a minimum transaction cost, mirroring the behavior applied to other transaction types.
