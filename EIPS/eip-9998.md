...
[EIP-9999](./eip-9999.md) extends `tx_floor` at runtime with the bytes that opcodes contribute to the [EIP-7928](./eip-7928.md) Block Access List, and adds a static per-auth term for the BAL contribution each authorization incurs. The mechanism is orthogonal to this EIP's static formula: EIP-9999 seeds its runtime accumulator with `tx_floor` and adds only the BAL-specific contributions on top.
