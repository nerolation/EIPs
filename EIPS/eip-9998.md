...
Floor impact by transaction type (mainnet sample, 1,500 random blocks, 441,271 transactions; 20,000 of those traced for [EIP-9999](./eip-9999.md) BAL bytes; May 2026):

| Type | Share | Avg content (B) | Old floor | New floor | % paying more (with EIP-9999) | Avg extra gas |
|---|---:|---:|---:|---:|---:|---:|
...
Across all transactions: 3.79% pay more, total network gas rises by 4.06%. EIP-9998 alone: 3.24% pay more, +3.29% gas. EIP-9999 adds another 0.54 pp and +0.77 pp.

Types 0, 1, 2 cover 98.3% of traffic and at most ~3.5% are affected. Typical users see no change. Type 3 (blob carriers, calldata-light) pays more 70% of the time, ~5,200 gas extra on average. Type 4 (SetCode) pays more 21% of the time, ~114,000 gas extra on average. That last group is the gap this EIP targets: auth tuples that pay no floor today.
...
An ERC-20 transfer (68 B calldata, ~50 k gas) has `tx_floor = 25,352`, well under `gas_used`. A 50 k-gas tx needs more than 453 B of calldata before the floor bites.
