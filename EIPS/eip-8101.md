...
MAX_CHUNKS_PER_BLOCK = 2**8  # Maximum chunks in a block (256 chunks per block)
MAX_TRANSACTIONS_PER_CHUNK = 2**16 # Maximum number of transactions in a chunk (65,536 transactions per chunk)
```
...
class ExecutionChunkHeader(Container):
    ...
    chunk_access_list_root: Root     # Merkle root of chunk access list
    ...
    transactions_root: Root          # Merkle root of transactions
    gas_used: uint64                 # Gas consumed in chunk
    blob_gas_used: uint64            # Blob gas consumed in chunk
    withdrawals_root: Root           # Merkle root of withdrawals (only in the last chunk)
...
class ExecutionChunk(Container):
    ...
    withdrawals: List[Withdrawal, MAX_WITHDRAWALS_PER_PAYLOAD]  # Only in the last chunk
```
...
AccountChanges = [
    Address,                    # address
    List[SlotChanges],          # storage_changes (slot -> [tx_index -> new_value])
    List[StorageKey],           # storage_reads (read-only storage keys)
    List[BalanceChange],        # balance_changes ([tx_index -> post_balance])
    List[NonceChange],          # nonce_changes ([tx_index -> new_nonce])
    List[CodeChange]            # code_changes ([tx_index -> new_code])
]

# ChunkAccessList: List of AccountChanges
